...
export default async function ShiftsPage() {
  ...
  const { data: incomingRequests } = await supabase
    ...
    .select(`id, status, created_at, requester:profiles!requester_id(full_name), requester_shift:shifts!requester_shift_id(shift_date, start_time, end_time)`)
    .eq("target_employee_id", user.id)
    .eq("status", "pending") as any;

  const { data: myRequests } = await supabase
    .from("shift_swap_requests")
    .select(`id, status, created_at, target_employee:profiles!target_employee_id(full_name), requester_shift:shifts!requester_shift_id(shift_date, start_time, end_time)`)
          ...
          <div className="space-y-3">
            {(incomingRequests as any[]).map((req: any) => (
              <div key={req.id} className="flex items-center justify-between p-3 bg-coffee-50 rounded-lg">
                <div>
                  <p className="font-medium text-gray-800">{req.requester?.full_name}</p>
                  <p className="text-sm text-gray-500">
                    Wants to swap their {req.requester_shift ? format(new Date(req.requester_shift.shift_date), "EEE MMM d") : ""} shift ({req.requester_shift?.start_time?.slice(0, 5)}–{req.requester_shift?.end_time?.slice(0, 5)})
                  </p>
                </div>
              ...
              <div key={req.id} className="flex items-center justify-between py-3 border-b border-gray-50 last:border-0">
                <div>
                  <p className="text-sm font-medium text-gray-800">
                    {req.requester_shift ? format(new Date(req.requester_shift.shift_date), "EEE MMM d") : "—"} ({req.requester_shift?.start_time?.slice(0,5)}–{req.requester_shift?.end_time?.slice(0,5)})
                  </p>
                  <p className="text-xs text-gray-400">{req.target_employee ? `→ ${req.target_employee.full_name}` : "→ Open to anyone"}</p>
                </div>
