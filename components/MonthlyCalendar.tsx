...
export default function MonthlyCalendar({
  ...
  const isNotPublished = scheduleStatus === null;

  return (
    <div>
      {/* Draft banner for managers */}
      {canManage && isDraft && (
        <div className="mb-4 px-4 py-2.5 bg-yellow-50 border border-yellow-200 rounded-xl text-sm text-yellow-800 flex items-center gap-2">
          <span>📝</span>
          <span>This schedule is saved as a <strong>draft</strong> — employees can&apos;t see it yet. Go to Manage to publish it.</span>
        </div>
      )}

      {/* Not yet published banner for non-managers */}
      {!canManage && isNotPublished && (
        <div className="bg-white rounded-xl p-12 text-center border border-gray-100">
          <p className="text-2xl mb-3">📅</p>
          <p className="text-gray-600 font-medium">Schedule not published yet</p>
          <p className="text-gray-400 text-sm mt-1">Your manager is still working on it. Check back soon.</p>
        </div>
      )}
      ...
      {(canManage || scheduleStatus === "published") && (
        <>
          {/* Month navigation */}
